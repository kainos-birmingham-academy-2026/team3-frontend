...
Distilled from *Kainos Brand and Visual Identity Guidelines, Version 1.0, April 2020*. For edge cases or detailed worked examples, consult `references/brand-guide.pdf`.
...
Fetched live from the Organization Assets **Office Templates** library (bundled fallback in `references/templates/`):
...
- **`sub-brand-logos/`** — 12 product and sub-brand lockups:
  - Kainos Smart, Smart-Audit, Smart-Shield, Smart-Test
  - Kainos Evolve
  - Kainos + Workday lockups (`Kainos_WD_White_lockup@2x.png`, `Workday_Logo.png`)
  - DavisPier_Kainos lockups (Blue, RGB, Reverse)
  - RAI-BOW lockup
  - PV_KW lockups
- **`icons/`** — curated essentials set (~30 concepts, 37 files) covering the most generally useful brand-approved icons. Most concepts have both a standard (`<Name>.png`) and reverse (`<Name> Rev.png`) variant for light vs dark backgrounds. Use only these for icon needs — never generic icon sets.
  - Bundled concepts: 2 People, Brain, Calendar, Cloud, Cog, Connect, Conversation, Document with magnifying glass, Email, Envelope, Heart, Lightbulb, Phone, Question, Rocket, Star, Target, Tick, Tools, Trophy.

**Not bundled — fetch on demand from SharePoint `OrganizationAssets` site:**
- Additional icons beyond the curated set → `Icons` library (full A–X catalogue, 296 files)
- Customer logos → `CustomerLogos` library
- Partner logos → `PartnerLogos` library
- Approved photography / illustrations → `KainosApprovedImagery`, `Illustrations`
- Awards and certifications → `AwardsAndCertifications`

Source of truth: `https://kainossoftwareltd.sharepoint.com/sites/OrganizationAssets`

---

*Source: Kainos Visual Identity Guidelines v1.0, April 2020 — see `references/brand-guide.pdf`.*
